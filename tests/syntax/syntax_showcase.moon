load "tests/syntax/dummy.moon" as dummy

let printTitle (title):
  show ""
  show "--- `title` ---"
end

printTitle "1. Variables & Primitives"
let number be 42
let decimal be 3.14
let truth be true
let lies be false
let nothing be nil
let string be "Hello, Moon!"
let a, b be 1, 2

show number
show decimal
show truth
show string
show a

printTitle "2. String Interpolation & Escapes"
let name be "Emrys"
let greeting be "Hello `name`! Welcome to Moon."
show greeting

let nested be "Outer `"Inner `100`"`"
show nested

let escaped be "Line 1\nLine 2\tTabbed\rCarriage\\Slash"
show escaped

printTitle "3. Collections & Access"
let list be [ 1, 2, 3, 4 ]
let user be {
  name: "Emrys",
  speed: "Fast"
}

show list
show user
show user["name"]
show user's speed

printTitle "4. Math, Logic, and Ranges"
let sum be 10 + 5
let diff be 10 - 5
let prod be 10 * 5
let quot be 10 / 2
let remainder be 10 mod 3

let isTrue be true and false or not false
let isEqual be 10 is 10
let isNotEqual be 10 is not 5
let isGreater be 10 > 5
let isGreaterEq be 10 >= 10
let isLessEq be 5 <= 10

let myRange be 1 to 10 by 2

show sum
show isTrue
show isGreater
show isGreaterEq
show isLessEq
show myRange

printTitle "5. Type Casting"
show 10 as String
show "42" as Number
show true as Number

printTitle "6. Custom Types & Instantiation"
type Player:
  name: "Unknown",
  health: 100
end

let p1 be Player with name: "Emrys"
end
show p1's name
show p1's health

printTitle "7. Control Flow & Chained Comparisons"
if 10 < 5:
  show "10 is less than 5"
else if 10 is 5:
  show "10 is exactly 5"
else:
  show "10 is indeed greater than 5"
end

unless false:
  show "Unless executes when false!"
end

let result be "Ternary" if true else "Fallback"
  show result

let expensive function:
  show "Evaluating expensive function!"
  give 50
end

if expensive function > 10 and < 100:
  show "Chained comparison worked and evaluated only once!"
end

let fetchCount be 0
let fetchValue:
  set fetchCount to fetchCount + 1
  give 50
end

if 10 < fetchValue <= 100:
  show "It works natively!"
end
show "fetchValue was called: " + fetchCount + " times (should be 1!)."

printTitle "8. Action Statements & Modifiers"
let counter be 0
add 5 to counter
set counter to 10
update counter * 2

show counter if counter > 10
  set counter to 0 unless counter is 0

printTitle "9. Loops & Control"
let i be 0
while i < 3:
  add 1 to i
  if i is 2:
    skip
  end
  show "While loop iteration"
end

let j be 0
until j is 3:
  add 1 to j
  show "Until loop iteration"
  quit if j is 2
end

for count from 1 to 3 by 1:
  show "For loop iteration"
end

for each item in list:
  show "For each item"
  break if item is 2
end

printTitle "10. Comprehensions & The it Construct"
let numbers be [ 1, 2, 3, 4, 5 ]
let doubled be [ for each n in numbers keep n * 2 if n > 2 ]

let users be [ {
    name: "Alice",
    age: 25
  }, {
    name: "Bob",
    age: 17
  } ]
let adults be [ for each u in users keep u if it's age > 18 ]

let dictComp be {
  for each x in numbers keep x: x * 10
}

show doubled
show adults
show dictComp

printTitle "11. Phrasal Functions & Union Types"
let push (item: Number or String) to (myList):
  show "Pushing to list..."
end

let jump:
  show "Jumping!"
end

jump
push 10 to list
push "hello" to list

printTitle "12. End Index & Advanced Slicing"
let slice be list[1 to end]
show slice

let backwards be list[end to 1]
show backwards

let midSlice be list[end / 2 to 1]
show midSlice

show "Showcase executed successfully!"
